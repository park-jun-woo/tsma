...
//ff:what promotePy: commits a passing Python loop test to its canonical path (match.CanonicalTestPath → test_<file>.py beside the source). Like promoteTS, it writes the ORIGINAL sanitized src (with its original module imports intact), not the sys.path-injected backing — the canonical file lives beside the source so pytest's prepend import mode resolves the original imports without the .tsma/test header. A write failure surfaces as TestFailed (a PASS that cannot persist is fed back as FAIL, never silent).
package tsmagate

import "github.com/park-jun-woo/tsma/internal/match"

// promotePy writes the original sanitized src to the canonical test path. The
// canonical sits in the source directory, so pytest resolves the original
// imports without the .tsma/test sys.path injection.
